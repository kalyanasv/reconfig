// Example using type - SV keyword.

module pkt_proc_type
  // Parameters for total packet size = 64 bits.
  // PACKET_DATA [DATA_SIZE] = PACKET[PARITY + FLAGS + ADDR + PAYLOAD]
  ...
  (  
   ...
   for (gi = 0; gi < PKT_PARSE_INSTANCES; gi++) begin: gen_pkt_inst
      ...
      // Auto assignment as names across modules match with I/O.
        payload_parse (.*, .pkt_valid (pkt_valid && (gi == pkt_ctl.id)), 
		       .pkt_ctl_data(pkt_ctl_data[gi]),
                       .pkt_payload_valid (pkt_payload_valid[gi]),
		       .pkt_payload_out (pkt_payload_out[gi]));
      
      /* Packet control parser uses related parameters 
       such as FLAGS and ADDR.*/
      pkt_ctl_proc_type
      // Auto assignment as names across modules match with I/O.
	#(.pkt_ctl_t(pkt_ctl_t))
